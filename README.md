...
For power users, the integration exposes 40+ commands via `vacuum.send_command` for operations like zone cleaning, per-room custom settings, cruise/patrol, media capture, and raw DPS access.

> [!TIP]
> Most common operations (start, stop, fan speed, dock tasks, schedules) are available as entities and don't need `send_command`. See the full reference for when you actually need it.

**[Full `send_command` Reference →](docs/SEND_COMMAND_REFERENCE.md)**
