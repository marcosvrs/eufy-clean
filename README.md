...
> [!NOTE]
> Most common operations are available directly via entities (switches, selects, buttons) without needing `send_command`. This reference is for **power users** who need advanced control or automation beyond what entities provide.
...
| `room_clean` | `room_ids` (list), `map_id` (int), `fan_speed` (str), `water_level` (str), `clean_times` (int) | Clean specific rooms. See [Cleaning Specific Rooms](#cleaning-specific-rooms). |
...
Override cleaning parameters per room in a single session:
...
Most dock operations are available as button entities. These commands are for automation:
...
This sends a raw DPS value without any encoding. Use with caution.
