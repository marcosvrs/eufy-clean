...
def test_missing_battery_data_handling(mock_coordinator):
    """Test that missing battery data returns None."""
    # Setup battery sensor with no battery data
    mock_coordinator.data.battery_level = None
...
def test_negative_battery_data_handling(mock_coordinator):
    """Test that negative battery data is treated as unavailable."""
    # Setup battery sensor with negative battery data
    mock_coordinator.data.battery_level = -1
...
def test_zero_battery_data_handling(mock_coordinator):
    """Test that zero battery level is valid."""
    # Setup battery sensor with zero battery level
    mock_coordinator.data.battery_level = 0

    entity = BatterySensorEntity(mock_coordinator)
    entity.hass = MagicMock()

    # Zero is a valid battery level
    assert entity.native_value == 0


def test_valid_battery_data_handling(mock_coordinator):
    """Test that valid battery data is returned correctly."""
    # Setup battery sensor with valid battery data
    mock_coordinator.data.battery_level = 75

    entity = BatterySensorEntity(mock_coordinator)
    entity.hass = MagicMock()

    # Should return the battery level
...
@pytest.mark.asyncio
async def test_mqtt_malformed_message_does_not_crash(mock_coordinator):
    ...
    # Simulate malformed MQTT messages
    malformed_messages = [
        b"not json",
        b"{}",
    ...
    for malformed_msg in malformed_messages:
        ...
        mock_coordinator._handle_mqtt_message(malformed_msg)
...
@pytest.mark.asyncio
async def test_mqtt_state_synchronization_timing(mock_coordinator):
    """Test that entity state synchronization happens immediately after MQTT update.
    
    Validates: Requirement 8.4 - When the device reports state changes via MQTT,
    then all entities shall update within 2 seconds (actually immediate in our implementation).
    """
    # Setup mopping device
    mock_coordinator.device_model = "T2150"  # G10 Hybrid
    ...
    # Simulate comprehensive MQTT message by directly updating coordinator
    mock_coordinator.data.fan_speed = "Turbo"
    mock_coordinator.data.battery_level = 65
    mock_coordinator.data.rooms = [
        {"id": 1, "name": "Kitchen"},
...
@pytest.mark.asyncio
async def test_mqtt_room_data_update_preserves_other_state(mock_coordinator):
    """Test that MQTT room data update doesn't affect other entity states.
    
    Validates: Requirement 1.5 - Room data updates are independent of other state.
    """
    # Create all entities
    vacuum_entity = RoboVacMQTTEntity(mock_coordinator)
    vacuum_entity.hass = MagicMock()
    ...
    # Simulate MQTT message updating only room data by directly updating coordinator
    mock_coordinator.data.rooms = [
        {"id": 1, "name": "Kitchen"}
    ]
...
# ============================================================================
# Additional MQTT Integration Tests (Requirements 1.5, 8.4)
# ============================================================================


@pytest.mark.asyncio
@pytest.mark.parametrize(
    "room_data,expected_rooms",
    [
        # Single room with integer ID (expected output uses str - normalised by _rooms_to_attributes)
        (
            [{"id": 1, "name": "Kitchen"}],
            [{"id": "1", "name": "Kitchen"}]
        ),
        # Multiple rooms with integer IDs
        (
            [
                {"id": 1, "name": "Kitchen"},
                {"id": 2, "name": "Living Room"},
        ...
        # Rooms with string IDs (already str - unchanged)
        (
            [
                {"id": "1", "name": "Kitchen"},
                {"id": "2", "name": "Living Room"}
        ...
        # Mixed ID types (all normalised to str)
        (
            [
                {"id": 1, "name": "Kitchen"},
                {"id": "2", "name": "Living Room"}
...
async def test_mqtt_room_data_integration(mock_coordinator, room_data, expected_rooms):
    """Test MQTT room data updates with various room configurations.
    ...
    # Set initial state with no rooms
    mock_coordinator.data.rooms = []
    assert vacuum_entity.extra_state_attributes["rooms"] == []
    
    # Simulate MQTT message with room data by directly updating coordinator
    # (This simulates what _handle_mqtt_message does after parsing)
    mock_coordinator.data.rooms = room_data
    
    # Verify rooms attribute reflects the new data
    assert vacuum_entity.extra_state_attributes["rooms"] == expected_rooms
    
    # Verify each room has correct structure
    for room in vacuum_entity.extra_state_attributes["rooms"]:
        assert "id" in room
        assert "name" in room
        assert isinstance(room["id"], (int, str))
        assert isinstance(room["name"], str)
