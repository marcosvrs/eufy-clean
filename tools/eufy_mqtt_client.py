...
try:
    ...
    for _pkg, _path in [
        ...
        (
            ...
            f"{_root}/custom_components/robovac_mqtt/proto",
        ),
        (
            "custom_components.robovac_mqtt.proto.cloud",
            f"{_root}/custom_components/robovac_mqtt/proto/cloud",
        ),
    ]:
...
def _decode_dps_for_display(key: str, value) -> str:
    ...
    if key == "153":
        try:
            ...
            if msg.HasField("go_wash"):
                ...
                parts.append(
                    f"go_wash={go_wash_names.get(msg.go_wash.mode, msg.go_wash.mode)}"
                )
            if msg.HasField("trigger"):
                ...
                parts.append(
                    f"trigger={trigger_names.get(msg.trigger.source, msg.trigger.source)}"
                )
            return " | ".join(parts)
        except Exception:
            return value[:60]
    ...
    if key == "173":
        ...
        if msg.HasField("status"):
            ...
            parts.append(
                f"dock={status_names.get(msg.status.state, f'state={msg.status.state}')}"
            )
    ...
    if key == "176":
        ...
        if msg.HasField("water_level_sw"):
            parts.append(
                f"water_level_sw={'on' if msg.water_level_sw.value else 'off'}"
            )
        if msg.HasField("unistate") and msg.unistate.HasField("mop_state"):
            parts.append(
                f"mop={'attached' if msg.unistate.mop_state.value else 'detached'}"
            )
    ...
    if key == "169":
        try:
            ...
            meaningful = [
                s.strip()
                for s in readable
                if not _re.fullmatch(r"[0-9a-f]+", s.lower())
            ]
...
class EufyCloudAuth:
    ...
    def get_mqtt_certs(self) -> dict:
        resp = requests.post(
            self.MQTT_INFO_URL, headers=self._aiot_headers(), json={}, timeout=15
        ).json()
        return resp.get("data", {})

    def get_devices(self) -> list:
        resp = requests.post(
            self.DEVICE_LIST_URL, headers=self._aiot_headers(), json={}, timeout=15
        ).json()
...
class EufyMqttClient:
    ...
    def _save_mqtt_message(self, data: dict):
        ...
        dps = payload.get("data", {})
        dps_keys = (
            "_".join(sorted(dps.keys(), key=lambda k: int(k))) if dps else "no_dps"
        )
...
async def main_async():
    ...
    parser.add_argument(
        "--email",
        default=os.environ.get("EUFY_EMAIL"),
        help="Eufy account email (or EUFY_EMAIL env)",
    )
    parser.add_argument(
        "--password",
        default=os.environ.get("EUFY_PASSWORD"),
    ...
    for d in cleaning_devices:
        dev = d["device"]
        print(
            f"  {dev['device_name']} ({dev['device_model']}) "
